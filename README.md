...
# Examples
 ...
 - Example production-like setup with a cluster of application nodes running on Docker swarm and monitored with Prometheus.
    ...
    1. Run the application in docker stack (deploys docker stack called `process`):
        ``` sh
    ...
    4. Remove the docker stack and its database volume with:
        ``` sh
        docker stack rm process
        docker volume rm process_db-data
        ```
...
## Tags
Metrics include teh following tags:
...
When running an applcaition in a cluster using a shared database, it is only necassary for one of the instances to be retreiving the guage metrics which provide the current snapshot from the database. However it is important that counter metrics are being monitored on all instances by the monitoring system. For example, `camunda.process.instances.started` metric will provide the count of process instances starts on the instance since it started, so to alert if a particular process start count drops below a threshold then all instances in the cluster need to be monitored.

The `camunda.monitoring.snapshot.enabled` Spring Boot property can be set to true on a sigle instance in the cluster.
